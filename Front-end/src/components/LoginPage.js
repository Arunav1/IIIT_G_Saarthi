...
function LoginPage() {
  ...
  const handleSignOut = () => {
    ...
    // remain on login page so user can sign in again
    window.location.reload();
  };

  useEffect(() => {
    // Do not auto-redirect if a token exists — show the login page so
    // the user can explicitly sign in with a different account if desired.
    // We still load the Google script and initialize the button below.
