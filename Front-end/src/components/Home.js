...
export default function Home() {
  ...
  // read logged-in user from localStorage (if any) into state so header updates when profile changes
  ...
  const toggleProfile = async () => {
    ...
    if (willOpen && !profile) {
      // fetch profile once when opening
      const token =
      ...
      try {
        ...
        if (res.data && res.data.user) {
          ...
          // also refresh storedUser state so header reflects latest name
          setStoredUser((prev) => ({
            ...(prev || {}),
            ...(res.data.user || {}),
          }));
